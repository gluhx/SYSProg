...
void print_help() {
    printf("Используйте: 03_tcp [options]\n\n");
    printf("Режимы работы:\n");
    printf("  03_tcp <port>              # TCP сервер (прослушивание порта)\n");
    printf("  03_tcp <ip> <port>         # TCP клиент (подключение к серверу)\n\n");
    printf("Примеры:\n");
    printf("  03_tcp 8888                 # TCP сервер на порту 8888\n");
    printf("  03_tcp 127.0.0.1 8888       # TCP клиент для подключения к 127.0.0.1:8888\n");
}
...
int main(int argc, char *argv[]) {
    ...
    if (args.is_server) {
        ...
        result = tcp_server_mode(args.port, key);
    } else {
        // Клиентский режим
        result = tcp_client_mode(args.ip, args.port);
    }
