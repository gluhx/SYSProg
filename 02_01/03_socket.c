...
int icmp_mode(char *ip) {
    ...
    if (sock_fd < 0) {
        perror("socket");
        printf("Для ICMP нужны root-права\n");
        return 1;
    }
    ...
    if (is_server) {
        ...
        if (icmp->type == ICMP_ECHO) {
            ...
            snprintf(data, 32, "KEY:%c\n", global_key);
            resp_icmp->checksum = icmp_checksum((unsigned short *)resp_icmp,
                                               sizeof(struct icmphdr) + strlen((char*)data));
            
            sendto(sock_fd, packet, sizeof(struct icmphdr) + strlen((char*)data), 0,
        ...
        sendto(sock_fd, packet, sizeof(struct icmphdr), 0,
               (struct sockaddr*)&peer_addr, sizeof(peer_addr));
        ...
        if (select(sock_fd+1, &fds, NULL, NULL, &tv) > 0) {
            ...
            if (bytes > 0) {
                struct iphdr *ip_hdr = (struct iphdr *)buf;
                struct icmphdr *resp_icmp = (struct icmphdr *)(buf + (ip_hdr->ihl * 4));
                char *data = (char *)(buf + (ip_hdr->ihl * 4) + sizeof(struct icmphdr));
                
                if (resp_icmp->type == ICMP_ECHOREPLY && 
                    strncmp(data, "KEY:", 4) == 0) {
                    global_key = data[4];
                    key_exchanged = 1;
                    printf("[Ключ: %c]\n", global_key);
                }
    ...
    // Специальная версия чата для ICMP
    char input_buffer[MAX_BUFF] = {0};
    int pos = 0;
    ...
    while (1) {
        ...
        if (select(sock_fd + 1, &read_fds, NULL, NULL, &tv) < 0) break;
        ...
        if (FD_ISSET(sock_fd, &read_fds)) {
            ...
            if (bytes > 0) {
                ...
                if (ntohs(icmp->un.echo.id) == ICMP_ID) {
                    ...
                    if (data_len > 0) {
                        // Для сервера обновляем адрес
                        if (is_server && icmp->type == ICMP_ECHO) {
                            memcpy(&peer_addr, &src, sizeof(peer_addr));
                        }
                        
                        // Расшифровываем
                        xor_cipher((unsigned char*)data, data_len);
                        
                        // Пропускаем KEY сообщения
                        if (strncmp((char*)data, "KEY:", 4) != 0) {
                            char *colon = strchr((char*)data, ':');
                            if (colon != NULL) {
                                *colon = '\0';
                                pid_t sender = atoi((char*)data);
                                char *msg = colon + 1;
            ...
            if (read(0, &c, 1) == 1) {
                if (c == '\n') {
                    if (pos > 0) {
                        ...
                        sendto(sock_fd, packet, sizeof(struct icmphdr) + msg_len, 0,
                               (struct sockaddr*)&peer_addr, sizeof(peer_addr));
