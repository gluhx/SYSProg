...
show_help() {
    ...
    echo ""
    echo "  -t, --time <time>     Время: время через которое после старта будет остановлен процесс (по умолчанию 30 секунд)"
    echo "  -h, --help"           
    exit 1
}
...
if [[ "$TIME_FLAG" == true ]]; then
    echo ""
    echo "Вы не выбрали опцию time - по умолчанию 30 секунд"
else
    echo ""
